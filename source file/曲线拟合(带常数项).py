...
x=np.array([[1,1,1],[1,2,4],[1,3,9],[1,4,16],[1,5,25]])
...
b0,b1,b2=beta #序列解包，作为多项式的系数,这里从低到高
...
调用polyfit拟合成二次多项式，可以检验Z1与Z2完全一样，多项式的系数,这里从高到低
z2 = np.polyfit(x,y,2)
...
y3=list(map(lambda item:b2*item**2+b1*item+b0,x3))
y3=np.array(y3)
...
#添加文本标注
plt.text(2,2.5,'-0.1357 x^2 + 1.344 x + 0.58')
